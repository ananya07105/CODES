...
using namespace std;
bool ispal(string input, int start, int end)
{
	while(start<end){
...
int minpal(string input,int i,int j, unordered_map<string,int> &memo)
{
	if(i>j)
	  return 0;
	...
	for(int k=i;k<j;k++)
	{
		...
		if(left_min==INT_MAX)
		  left_min=minpal(input,i,k,memo);
		if(right_min==INT_MAX)
		  right_min=minpal(input,k+1,j,memo);
...
int main()
{
	...
	cout<<minpal(input,0,input.length()-1,memo)<<endl;
	return 0;
}
